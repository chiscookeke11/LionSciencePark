...
export default function NewsCard({ data }: RecentBlogCardProps) {
    return (
        <Link href={`/news/${data.id}`} className="w-full h-[430px] max-w-xs  md:max-w-sm min-h-[300px]   rounded-xl  border-2 border-black relative z-0 group text-[#0a0a0a] " >
            ...
            <div className="flex flex-col items-center justify-between w-full h-full overflow-hidden gap-5 rounded-xl absolute top-0 left-0 z-20 bg-white " >
                <div className=" w-full bg-gray-300 h-[50%] border-b border-b-black overflow-hidden " >
                    <CldImage src={data.image ?? "/placeholder-image.svg"} alt="image" height={500} width={500} crop={{ type: "auto", source: true }} className="object-cover object-center h-full w-full " />
                </div>
